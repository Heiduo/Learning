...
//        System.out.println("result: " + nthUglyNumber(4));
         ...
         * 229. 求众数 II
         */
//        System.out.println("result: " + majorityElementOther(new int[]{1,1,1,3,3,2,2,2}));

        /**
         * 299. 猜数字游戏
         */
        System.out.println("result: " + getHintMine("1123","7810"));
    }
     ...
     * @return
     */
    public static String getHintMine(String secret, String guess) {
        char[] secChar = secret.toCharArray();
        char[] gueChar = guess.toCharArray();
        int[] secInt = new int[10];
        int[] gueInt  =new int[10];
        int bulls = 0;
        int cows = 0;
        int count0 = '0';
        for (int i = 0; i < secChar.length; i++) {
            if (secChar[i] == gueChar[i]){
                bulls++;
                secChar[i] = gueChar[i]=0;
            }else {
                secInt[secChar[i]-count0]++;
                gueInt[gueChar[i]-count0]++;
            }
        }
        for (int i = 0; i < secInt.length; i++) {
            cows+=Math.min(secInt[i],gueInt[i]);
        }
    ...
    public static List<Integer> majorityElementOther(int[] nums) {
    ...
    public static List<Integer> majorityElementMine(int[] nums) {
        List<Integer> list = new ArrayList<>();
     ...
     * @return
     */
    public static int strangePrinterMine(String s) {
